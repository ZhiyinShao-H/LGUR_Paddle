...
class CUHKPEDEDataset(paddle.io.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = paddle.to_tensor(np.array([self.label[index]])).long()
    ...
    def caption_mask(self, caption):
        ...
        caption = paddle.to_tensor(np.array(caption)).view(-1).float()
        if caption_length < self.opt.caption_length_max:
            zero_padding = paddle.zeros(self.opt.caption_length_max - caption_length)
            caption = paddle.concat([caption, zero_padding], 0)
        else:
...
class CUHKPEDE_img_dateset(paddle.io.Dataset):
    def __init__(self, opt, tran):
...
class CUHKPEDE_txt_dateset(paddle.io.Dataset):
